...
public class ReportsController : ControllerBase
{
    ...
    {
        var result = await _mediator.Send(new GetDailyRevenueQuery(date ?? DateTime.UtcNow.Date));
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GetMonthlyRevenue([FromQuery] int? year, [FromQuery] int? month)
    {
        ...
        var result = await _mediator.Send(new GetMonthlyRevenueQuery(year ?? now.Year, month ?? now.Month));
        return Ok(result);
    }
    ...
    {
        var result = await _mediator.Send(new GetYearlyRevenueQuery(year ?? DateTime.UtcNow.Year));
        return Ok(result);
    }
